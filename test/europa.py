...
M = 2
L = np.arange(M if M>1 else 3, M+10, 2)

# Prepare solver 
Rc      = (R - H)*0.5        # core radius
Rp      = R             # body radius
a       = Rc/Rp*np.pi
...
def my_plot2():
    ...
    disp_grd_cm = np.real(disp_grd)/100
    vmin = np.min(disp_grd_cm)
    vmax = np.max(disp_grd_cm)

    fig, ax = plt.subplots(subplot_kw=dict(projection='polar'))
    #cb = ax.contourf(th_grd, x_grd, disp_grd_cm, levels=500,vmin=0,vmax=30,  cmap='inferno')#'YlOrBr'
    cb = ax.contourf(th_grd, x_grd, disp_grd_cm, levels=500, cmap='inferno')#'YlOrBr'
    ax.yaxis.grid(False)
    ax.xaxis.grid(False)
    ax.axis("off")
    ...
    #colorb2.set_label('m')
    '''
    plt.savefig('/Users/benja/Documents/projects/europa/test_fig2.png', dpi=1200)
    plt.show()
...
def get_N2_vs_dk2():
    N2_vec = np.linspace(1e-6, 1e-8, 100)
    dk2_vec = np.zeros(len(N2_vec))
    dk2_vec2 = np.zeros(len(N2_vec))
    Q_vec = np.zeros(len(N2_vec))

    for i in np.arange(len(N2_vec)):

        print(i)
        dyn = dynamical(cheb, ome, Om, ms, Mj, sma, Rp, 
                    rho=rhow, rhoc=rhoc, Rc=Rc, N2=N2_vec[i],
                    l=L, m=M, tau=tau, x1=a, x2=b,
                        tides='ee', e=e)
    ...
    return N2_vec, dk2_vec, dk2_vec2, Q_vec
